...
function bp_get_page_title() {
	...
	if ( is_front_page() || ( is_home() && bp_is_page( 'home' ) ) ) {
		...
		$title = __( 'Create a Profile', 'buddypress' );

	} else if ( bp_is_activation_page() ) {
		$title = __( 'Activate your Profile', 'buddypress' );
...
function bp_displayed_user_bio() {
	if ($bio = xprofile_get_field_data('Bio', bp_displayed_user_id()))
		echo "<h3>$bio</h3>";
}
