...
function relevanssi_search($q, $cat = NULL, $excat = NULL, $expost = NULL, $post_type = NULL, $taxonomy = NULL, $taxonomy_term = NULL, $operator = "AND", $search_blogs = NULL, $customfield_key = NULL, $customfield_value = NULL) {
	...
	do {
		foreach ($terms as $term) {
			...
			$query = "SELECT doc, term, tf, title, type, item FROM $relevanssi_table WHERE $term_cond $query_restrictions";
			...
			foreach ($matches as $match) {
				...
				switch ($match->title) {
					...
					case "3":
						...
						break;
					case "7":
						$weight = $weight * $link_boost;
						isset($link_matches[$match->doc]) ? $link_matches[$match->doc] += $match->tf : $link_matches[$match->doc] = $match->tf;
						$term_hits[$match->doc][$term] += $match->tf;
						break;
					default:
...
function relevanssi_index_user($user, $remove_first = false) {
	...
	if (isset($user->description)) {
		...
		foreach($tokens as $term => $tf) {
			$wpdb->query("INSERT INTO $relevanssi_table (doc, term, tf, title, type, item)
			VALUES (-1, '$term', $tf, 6, 'user', $user->ID)");
		}
	}

	if (isset($user->first_name)) {
		$wpdb->query("INSERT INTO $relevanssi_table (doc, term, tf, title, type, item)
		VALUES (-1, '$user->first_name', 1, 6, 'user', $user->ID)");
	}

	if (isset($user->last_name)) {
		$wpdb->query("INSERT INTO $relevanssi_table (doc, term, tf, title, type, item)
		VALUES (-1, '$user->last_name', 1, 6, 'user', $user->ID)");
	}
	...
	if (isset($user->display_name)) {
		...
		foreach($parts as $part) {
			$wpdb->query("INSERT INTO $relevanssi_table (doc, term, tf, title, type, item)
			VALUES (-1, '$part', 1, 6, 'user', $user->ID)");
		}
	}
}
...
function relevanssi_remove_doc($id, $keep_internal_links = false) {
	...
	$and = $keep_internal_links ? 'AND title != 7' : '';
				...
				if ( !empty( $link_id ) ) {
					...
					if ( count( $link_words > 0 ) ) {
						foreach ( $link_words as $word => $count ) {
							...
							$n++;
							$wpdb->query("INSERT INTO $relevanssi_table (doc, term, tf, title, item)
							VALUES ($link_id, '$word', $count, 7, $post->ID)");
						}
					}
				}
			}
...
function relevanssi_options_form() {
	global $title_boost_default, $tag_boost_default, $link_boost_default, $comment_boost_default, $wpdb, $relevanssi_table;
...
<div class='postbox-container' style='width:70%;'>
	...
	<small><?php printf(__('Default: %d', 'relevanssi'), $link_boost_default); ?></small>
	<br />
