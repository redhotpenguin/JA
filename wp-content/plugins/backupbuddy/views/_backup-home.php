...
if ( is_multisite() && pb_backupbuddy::$classes['core']->is_network_activated() && !defined( 'PB_DEMO_MODE' ) ) { // Multisite installation.
	pb_backupbuddy::$ui->title( 'Backup Network (BETA)' );
	pb_backupbuddy::alert( 'BETA WARNING:
	All BackupBuddy Multisite functionality is in BETA. Use of it on production sites is at your own risk.
	Multisite <i>Importing & Exporting functionalities are quite solid</i> (though still beta).
	Network migrations however have several known major complications;
	if you attempt to migrate an entire Network to a new URL or server you
	should expect to encounter difficulties. Primarily: If you are migrating
	a Network to a new URL, the subsite URLs will not be automatically
	migrated for you. You may use the Server Information page\'s database
	replacement tool to manually upgrade each URL. See the <a href="http://ithemes.com/codex/page/BackupBuddy_Multisite">BackupBuddy Multisite Knowledge Base</a> for additional information.' );
} else {
