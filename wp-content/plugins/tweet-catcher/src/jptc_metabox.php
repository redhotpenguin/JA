...
class Tweet_Manager_Metabox{
    ...
    public function __construct($jptc_model, $plugin_url){
		...
		add_action( 'save_post', array($this,'jptc_save_postdata'), 1, 2);
       // add_action( 'save_post', array($this,'jptc_save_postdata') );
		add_action( 'add_meta_boxes', array( &$this, 'add_metabox' ) );
		add_action('admin_head', array(&$this, 'add_js'));
		add_action('admin_head', array($this,'add_css'));
		add_action('wp_ajax_my_jptc' , array($this,'get_tweets_stream_callback'));
    }
	...
	/*  Even though non admins can't load this page, they can still POST data to /wp-admin/admin-ajax.php
		Let's make sure only real admin can execute this callback.
	*/
	...
	$_function = $_POST['_function'];
			switch($_function){
				...
				case 'admin_page_delete_tweet':
					echo $this->jptc_model->delete_tweet($_POST);
				break;
				
				case 'admin_page_more_tweets':
					@session_start();
					if( isset($_SESSION['jptc_current_tweets_displayed']) ){
						$offset = $_SESSION['jptc_current_tweets_displayed'];
						$_SESSION['jptc_current_tweets_displayed'] = $offset + JPTC_ADMIN_DISPLAY_N_TWEETS;
					}
					
					else {
						$offset = $_SESSION['jptc_current_tweets_displayed'] = 0;
					}

					echo $this->jptc_model->the_last_saved_tweets(JPTC_ADMIN_DISPLAY_N_TWEETS, 0, $offset);
	...
	public function jptc_save_postdata($post_id, $post){
	...
	if ($post->post_type == 'revision') {
		...
		if(empty($rt_query))
			delete_post_meta($post->ID, 'jptc_rt_query');

		else
			update_post_meta($post_id, 'jptc_rt_query', $rt_query);
		
		if(empty($rt_count) && is_numeric($rt_count)) update_post_meta($post_id, 'jptc_rt_count', abs($rt_count));
	...
	public function metabox_content($post){
		...
		<div id="saved_tweets_wrapper">
			<div id='saved_tweets_box'>
				<input type="hidden" id='jptc_post_id' value="<?php echo $post->ID;?>" /> 
