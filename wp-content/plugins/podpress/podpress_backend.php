...
if ( !defined('DB_NAME') ) { // everything is normal
	...
	podPress_var_dump('podpress_backend.php - param filename: ');
	podPress_var_dump($_GET['filename']);
	
	switch( strtolower($_GET['action']) ) {
		case 'id3image':
			podPress_isAuthorized('edit_posts');
			podPress_getCoverArt(rawurldecode($_GET['filename']));
		break;
		case 'getduration':
			podPress_isAuthorized('edit_posts');
			echo podPress_getDuration(rawurldecode($_GET['filename']));
		break;
		case 'getfilesize':
			podPress_isAuthorized('edit_posts');
			echo podPress_getFileSize(rawurldecode($_GET['filename']));
		break;
		case 'showid3contents':
			podPress_isAuthorized('edit_posts');
			echo podPress_showID3tags(rawurldecode($_GET['filename']));
		break;
		case 'streamfile':
