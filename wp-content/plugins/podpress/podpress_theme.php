	...
	function podPress_webContent($podPressTemplateData) {
		...
		foreach ($podPressTemplateData['files'] as $key=>$val) {
			...
			if(isset($val['image'])) {
				if($val['enableDownload'] && !empty($val['URI'])) {
					if ( 0 === strpos($val['type'], 'embed_') ) {
						$podPressDownloadlinks .= '<a href="'.$val['URI'].'"'.$target_attribute.' title="'.sprintf(__('Direct Link to %1$s', 'podpress'), $podPressEpisodeTitle).'">';
					} else {
						$podPressDownloadlinks .= '<a href="'.$val['URI'].'"'.$target_attribute.' title="'.sprintf(__('Download: %1$s', 'podpress'), $podPressEpisodeTitle).'">';
					}
				}
			...
			if(TRUE == isset($val['enableTorrentDownload']) AND TRUE === $val['enableTorrentDownload']) {
				$podPressDownloadlinks .= '<a href="'.$val['URI_torrent'].'"'.$target_attribute.' title="'.sprintf(__('Download: %1$s - .torrent file', 'podpress'), $podPressEpisodeTitle).'">';
