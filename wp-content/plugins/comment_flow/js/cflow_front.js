jQuery(document).ready( function($) {
    ...
    function submit_comment_callback( data, textStatus ){
        ...
        switch(data.depth){
            ...
            case 2:
                target = $( '#comment-' + data.cpid ).find('.children:first');
                if(target.length < 1 ){ // first reply to a thread, need to build ul.children
                    target = $("#comment-"+data.cpid).append('<ul class="children"></ul>').find('ul');
                }
            ...
            case 3:
                target = $( '#comment-' + data.cpid ).find('.children');
                if(target.length < 1 ){ // first reply to a reply, need to build ul.children
                    target = $("#comment-"+data.cpid).append('<ul class="children"></ul>').find('ul');
                }
        
                target.append(data.comment);
                add_reply_btn_to_children(data.comment_id);
                break;
        }
