...
function rpx_process_token(){
  ...
  if ( $rpx_http_vars['action'] == RPX_TOKEN_ACTION && !empty($rpx_http_vars['token']) ){
...
function rpx_process_user($action){/*Using a switch for visual clarity, this may be cleaned up when conditionals are no longer in flux.*/
  ...
  switch ($action){
    ...
    case 'regdirect':
      ...
      break;
    case 'getemail': // executed when the provider didn't return an email (twitter, linkedin)
      ...
      return false;
      break;
    case 'retryemail': // executed when email is already existing
...
function rpx_test_wp_user(){
  ...
  if ($rpx_match === true && $rpx_locked === false){
    return 'signon';
  }

  if ($active_user === true && $rpx_locked === false){
    return 'engage';
  }

  if ($rpx_match === true && $rpx_locked === true && ($rpx_email === false || $email_found === true || $rpx_valid === false)){
    return 'retryemail';
  }

  if ($rpx_match === true && $rpx_locked === true && $rpx_email === true && $email_found === false){
    return 'register';
  }

  if ($email_found === true && $rpx_verified_email === true && $rpx_locked === false && get_option(RPX_VEMAIL_OPTION) == 'true'){
    return 'vemail';
  }

  if ($autoreg === true && $rpx_match === false && $rpx_email === true && $email_found === false && $rpx_username === true && $username_match === false){
    return 'create';
  }

  if ($autoreg === false && $rpx_match === false){
    return 'regdirect';
  }

  if ($autoreg === true && $rpx_match === false  && ($rpx_username === false || $username_match === true)){
    return 'getuser';
  }

  if ($autoreg === true && $rpx_match === false && ($rpx_email === false || $email_found === true)){
    return 'getemail';
  }
...
function rpx_register_wp_user($collect='email'){
  ...
  $url = $reg_url.$connect.'action='.RPX_REGISTER_FORM_ACTION.
    ...
    '&redirect_to='.urlencode($rpx_http_vars['redirect_to']).
	'&rpx_collect='.$collect;
...
function rpx_signon_wp_user($redirect=""){
...
function rpx_update_profile_info($user_id){
	...
	switch($provider){
	    case 'Twitter':  
	      ...
	      update_twitter_information($user_id); 
	      break;
	    ...
	    case 'LinkedIn':
	      ...
	      update_linkedin_information($user_id);
	      break;
...
function  rpx_send_admin_notification($user){
	$username = $user->user_login;
	$user_email = $user->user_email;
	$user_provider = $user->rpx_provider;
	$user_display_name = $user->display_name;
	
	$subject = "New User Registration - $user_display_name";
		
	$user_login = bp_core_get_username($user->id);
	$site_url = get_site_url();
	$user_profile_url = $site_url."/members/$user_login/";
	$user_edit_profile = $user_profile_url.'profile/edit/group/1/';
	...
	$user_website = xprofile_get_field_data('Website' ,$user_id);

	$count = bp_core_get_total_member_count();
	$total_users = 'Congratulation! <b>'. $count.'</b> users are now registered on Journalism Accelerator.';
	...
	$message  = "$user_display_name has registered on Journalism Accelerator: <br/><br/>";
	$message .= "<h3>Basic info:</h3>Name: $user_display_name <br/>";
	$message .= "Username: $user_login (ID #$user_id)<br/>";
...
function rpx_send_user_notification($user){
	...
	$subject = "Welcome to the JA";
	$message .= "Thank you for registering on <a href='http://www.journalismaccelerator.com/'>Journalism Accelerator</a>.<br/><br/>";
	$message .= "Please find below the profile information you provided when you created your account.  You can login anytime to update your profile, make a comment, or answer a question. ";
	$message .="Don't forget to fill out your <a href='$user_edit_profile'>profile</a> with information about you and your area of expertise.";
	
	$message .= "<h3>Basic info:</h3>";
	if($user_display_name) $message.= "Name: $user_display_name <br/>";
	$message .= "Username: $user_login<br/>";
	$message .= "Email: $user_email <br/>";
	$message .= "JA Profile: <a href='$user_profile_url'>$user_login</a> ";

	if($user_bio || $user_twitter || $user_website)$message .= "<br/><br/><h3>Extra info:</h3>";
	if($user_bio) $message .= "Twitter: $user_twitter <br/>";
...
/*no point in trying to catch errors since empty fields return false*/
function rpx_get_meta() {
  global $rpx_wp_profile;
...
function rpx_get_buddy_photo($wp_id) {
    $result = rpx_get_user_meta($wp_id, RPX_META_PHOTO);
    return $result;
}

/*wrapper for Wordpress update to avoid returning false on updates that match current values*/
function rpx_update_user_meta($wp_id, $meta_label, $value) {
